<script lang="ts">
	...
	import { IsMobile } from '@slyng/ui/sidebar';
	import { api } from '@slyng/app-core/api';
	import { subscribeChannels } from '@slyng/app-core/stores/ws.svelte';
	...
	import { getProfile, displayName } from '@slyng/app-core/stores/profiles.svelte';
		...
		<div class="flex h-12 shrink-0 items-center justify-between border-b border-border px-4">
			...
			<div class="flex items-center gap-2">
				<Tooltip.Root>
					...
					<Tooltip.Content side="bottom">Search messages</Tooltip.Content>
				</Tooltip.Root>
				<Tooltip.Root>
					<Tooltip.Trigger>
						{#snippet child({ props })}
