<script lang="ts">
	...
	let rows = $state<TrashedChannel[]>([]);
	let loading = $state(true);
	let pendingHard = $state<TrashedChannel | null>(null);

	async function refresh() {
		loading = true;
		try {
			rows = await api.servers.trashChannels(serverId);
		} catch (err) {
			toast.error(err instanceof Error ? err.message : 'Failed to load trashed channels');
		}
		loading = false;
	}
	...
	async function restore(row: TrashedChannel) {
		try {
			await api.channels.restore(row.id);
	...
	async function hardDelete(row: TrashedChannel) {
		try {
			await api.channels.hardDelete(row.id);
...
{:else if rows.length === 0}
	<p class="rounded-md border border-dashed border-border bg-muted/30 p-6 text-center text-sm text-muted-foreground">
		Trash is empty. Deleted channels show up here so you can restore them.
	</p>
{:else}
		...
		<table class="w-full text-sm">
			<thead class="bg-muted/40 text-xs uppercase tracking-wide text-muted-foreground">
				<tr>
					...
					<th class="px-3 py-2 text-left font-medium">Messages</th>
			...
			<tbody>
				{#each rows as row (row.id)}
					...
					{@const TypeIcon = row.type === 'voice' ? Volume2 : Hash}
					<tr class="border-t border-border">
						...
						<td class="px-3 py-2 text-muted-foreground">{row.message_count}</td>
						<td class="px-3 py-2">
							<div class="flex items-center gap-2">
...
{#if pendingHard}
	<HardDeleteConfirmDialog
		...
		count={pendingHard.message_count}
		onConfirm={async () => {
			const r = pendingHard;
