//! # slyng-types
//!
//! Shared data model for the Slyng chat platform. This crate defines every
...
//!
//! - **`#[derive(Serialize, Deserialize)]`** — `slyng-client` uses these
...
//!   that replaces hand-authored TS schemas in `@slyng/types`.
//! - **`#[derive(Tsify)]`** (from [`tsify_next`], wasm-only) — auto-derives
//!   matching TypeScript interfaces in the wasm-pack `.d.ts` so JS callers
//!   of `@slyng/client`'s WASM build get full type information without any
