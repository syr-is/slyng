...
//! type for the API entities defined in `slyng-types`.
//!
//! Run via:
//!
//! ```bash
//! cargo run -p slyng-types --bin generate-zod -- packages/ts/types/src/generated.ts
//! ```
//!
//! `@slyng/types`'s build script invokes this in CI so the .ts file
