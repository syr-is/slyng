...
//! `slyng-types`. The structs carry `#[derive(Tsify)]
...
impl Client {
	...
	pub fn new(
		...
		let key = session_key.unwrap_or_else(|| "slyng_session".to_string());
	...
	pub async fn update_invite(
		...
		code: String,
		data: JsValue,
	) -> std::result::Result<slyng_types::ServerInvite, JsValue> {
		let body: serde_json::Value = body_from_jsv(data)?;
		self.inner
	...
	pub async fn friend_send(
		...
		user_id: String,
		syr_instance_url: Option<String>,
	) -> std::result::Result<slyng_types::Friendship, JsValue> {
		self.inner
...
impl Realtime {
	...
	/// dispatches by `op` (matching `WsOp` from `@slyng/types`).
