 ...
 *
 * - `parseFrame` — whole-frame parse, for payloads that are scalars only
 *   (IDENTIFY, TYPING_START). One field, one meaning; there is nothing to
 *   salvage out of a frame that misses it.
...
/**
 * A frame's `d` as an object, spelling "no payload" the way this codebase's own
 * client spells it.
 *
 ...
 * one. Ops whose fields are all optional have to keep seeing it that way:
 ...
 * Ops with a required field fail on the missing field either way.
 */
...
export function parseFrame<T>(schema: z.ZodType<T>, d: unknown): FrameCheck<T> {
	const parsed = schema.safeParse(asBody(d));
