 ...
 *  - `slyng://auth/callback` — the mobile native app's deep-link
...
function isAllowedRedirect(value: unknown): value is string {
	...
	if (/^slyng:\/\/auth\/callback(?:\?[^\s]*)?$/.test(value)) return true;
...
/**
 * Read the active session id from either the `slyng_session` cookie
...
export class AuthController {
	...
	async callback(
		...
		try {
			...
			//
			// The param name is intentionally namespaced (`slyng_bridge`)
			...
			target = `${target}${sep}slyng_bridge=${encodeURIComponent(bridge)}`;
			...
			// (with `?slyng_bridge=…`), and emits a default success
			// page. No custom HTML needed here.
			if (target.startsWith('http://localhost:')) {
				return res.redirect(target);
			}

			// Mobile custom scheme (`slyng://auth/callback?slyng_bridge=…`).
			...
			const codeParam = (() => {
				try {
					return new URL(target).searchParams.get('slyng_bridge') ?? '';
				} catch {
					return '';
				}
			})();
			...
			const intentUrl =
				`intent://auth/callback?slyng_bridge=${encodeURIComponent(codeParam)}` +
				`#Intent;scheme=slyng;package=is.syr.slyng;` +
...
<a id="open" class="btn" href=${escapedDirect}>Open Slyng</a>
