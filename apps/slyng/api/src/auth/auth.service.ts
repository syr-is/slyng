...
export class AuthService {
	...
	private getStateSecret(): string {
		// SLYNG_STATE_SECRET is the stable HMAC key. Fall back to the
		// session secret or a process-lifetime random — the fallback breaks
		// state validation across restarts but keeps dev usable.
		let secret = this.config.get<string>('SLYNG_STATE_SECRET');
		if (secret) return secret;
		secret = this.config.get<string>('SLYNG_SESSION_SECRET');
		if (secret) return secret;
		if (!this._fallbackSecret) {
			this._fallbackSecret = randomBytes(32).toString('hex');
			this.logger.warn(
				'No SLYNG_STATE_SECRET / SLYNG_SESSION_SECRET set; using a process-lifetime fallback. OAuth state validation will reset on every restart.'
			);
		}
	 ...
	 * redirect target is a custom scheme (e.g. `slyng://`) so the
