...
export class IdpJwtService {
	...
	private getSecret(): string {
		const secret =
			this.config.get<string>('SLYNG_JWT_SECRET') ??
			this.config.get<string>('SLYNG_SESSION_SECRET');
		if (secret) return secret;
		if (this.config.get('NODE_ENV', 'development') === 'production') {
			throw new Error('SLYNG_JWT_SECRET must be set in production');
		}
		if (!this._fallbackSecret) {
			this._fallbackSecret = randomBytes(32).toString('hex');
			this.logger.warn(
				'No SLYNG_JWT_SECRET set; using a process-lifetime fallback. Issued tokens will not survive a restart.'
			);
		}
