...
import { buildDidDocument, isValidSyrDid, type DidDocument } from '@slyng/idp-crypto';
import { extractDid, extractLocalId } from '@slyng/types';
import type {
	PublicProfileData,
 ...
 * stay wire-compatible: slyng's own federation stores consume them.
 */
@Injectable()
export class IdpPublicService {
	...
	buildInstanceManifest(): SyrInstanceManifest {
		...
		return {
			...
			// Self-custody device flows slyng implements (P10). slyng signs
	 ...
	 * (live) emoji only, ordered by shortcode. Shape matches slyng's emoji store.
	 */
	 ...
	 * only, newest first, with an optional tag `search`. Shape matches slyng's
	 * gif store.
	 */
