...
export class RelationController {
	...
	async snapshot(@Req() req: AuthedRequest) {
	...
	async listFriends(@PaginatedQuery() p: PaginationOptions, @Req() req: AuthedRequest) {
		const userId = req.user?.id;
		if (!userId) throw new HttpException('Unauthorized', 401);
	...
	async sendRequest(@Body() body: FriendSendDto, @Req() req: AuthedRequest) {
		const userId = req.user?.id;
		if (!userId) throw new HttpException('Unauthorized', 401);
	...
	async accept(@Param('userId') requester: string, @Req() req: AuthedRequest) {
	...
	async decline(@Param('userId') requester: string, @Req() req: AuthedRequest) {
		const actor = req.user?.id;
		if (!actor) throw new HttpException('Unauthorized', 401);
		try {
	...
	async removeOrCancel(@Param('userId') other: string, @Req() req: AuthedRequest) {
		const actor = req.user?.id;
		if (!actor) throw new HttpException('Unauthorized', 401);
	...
	async listBlocked(@PaginatedQuery() p: PaginationOptions, @Req() req: AuthedRequest) {
		const userId = req.user?.id;
		if (!userId) throw new HttpException('Unauthorized', 401);
	...
	async block(@Body() body: BlockUserDto, @Req() req: AuthedRequest) {
		const userId = req.user?.id;
		if (!userId) throw new HttpException('Unauthorized', 401);
		if (!body?.user_id) throw new HttpException('user_id required', 400);
		try {
	...
	async unblock(@Param('userId') target: string, @Req() req: AuthedRequest) {
		const userId = req.user?.id;
		if (!userId) throw new HttpException('Unauthorized', 401);
		try {
	...
	async listIgnored(@PaginatedQuery() p: PaginationOptions, @Req() req: AuthedRequest) {
		const userId = req.user?.id;
		if (!userId) throw new HttpException('Unauthorized', 401);
	...
	async ignore(@Body() body: IgnoreUserDto, @Req() req: AuthedRequest) {
		const userId = req.user?.id;
		if (!userId) throw new HttpException('Unauthorized', 401);
		if (!body?.user_id) throw new HttpException('user_id required', 400);
		try {
	...
	async unignore(@Param('userId') target: string, @Req() req: AuthedRequest) {
		const userId = req.user?.id;
		if (!userId) throw new HttpException('Unauthorized', 401);
		try {
