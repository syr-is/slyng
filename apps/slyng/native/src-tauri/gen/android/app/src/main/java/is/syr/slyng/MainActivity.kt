...
class MainActivity : TauriActivity() {
  ...
  override fun onWebViewCreate(webView: WebView) {
      ...
      if (top > 0 || bottom > 0 || left > 0 || right > 0) {
        insetsJs = """
          (function () {
            ...
            d.style.setProperty('--slyng-sai-top', '${top}px');
            d.style.setProperty('--slyng-sai-right', '${right}px');
            d.style.setProperty('--slyng-sai-bottom', '${bottom}px');
            d.style.setProperty('--slyng-sai-left', '${left}px');
          })();
        """.trimIndent()
    ...
    // has exhausted, leaving the document with no `--slyng-sai-*` set.
  ...
  // 3s window — leaving the document without `--slyng-sai-*` set until
