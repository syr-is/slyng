...
pub fn run() {
	...
	// Mobile: register the slyng:// scheme via the deep-link plugin so
...
(function () {
  var apply = function () {
    var probe = document.getElementById('__slyng_sai_probe');
    if (!probe) {
      probe = document.createElement('div');
      probe.id = '__slyng_sai_probe';
      probe.style.cssText =
    ...
    d.style.setProperty('--slyng-sai-top', s.paddingTop);
    d.style.setProperty('--slyng-sai-right', s.paddingRight);
    d.style.setProperty('--slyng-sai-bottom', s.paddingBottom);
    d.style.setProperty('--slyng-sai-left', s.paddingLeft);
  };
  window.__slyngSaiPoll = apply;
