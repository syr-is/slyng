...
//!   full URL (which carries the bridge `?slyng_bridge=…`).
//!
//! - **Mobile** (`tauri-plugin-deep-link`): we register `slyng://`
//!   at the OS level. The API redirects to
//!   `slyng://auth/callback?slyng_bridge=…`, the OS routes it into
//!   Tauri, and the deep-link plugin fires our on_open_url handler.
//!
//! Both code paths funnel into [`handle_callback_url`], which parses
//! the bridge code, calls `slyng_client::login_complete`, persists
//! the session, and emits `auth-changed` for the JS-side login form.

use crate::session_store::TauriStoreSession;
use std::sync::Arc;
use slyng_client::{Client, LoginResponse};
use tauri::{AppHandle, Emitter, Manager, Runtime, State};
use tauri_plugin_opener::OpenerExt;
use tokio::sync::Mutex;

#[cfg(any(target_os = "android", target_os = "ios"))]
pub const MOBILE_REDIRECT_URI: &str = "slyng://auth/callback";
...
/// for a real session via slyng-client, and emit the result. Same
...
pub fn handle_callback_url<R: Runtime>(app: &AppHandle<R>, url_str: &str) {
	...
	// namespaced `?slyng_bridge=…` and the legacy `?code=…`. Either
	...
	let code = parsed.as_ref().and_then(|u| {
		u.query_pairs()
			.find(|(k, _)| k == "slyng_bridge" || k == "code")
