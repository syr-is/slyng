//! Tauri command wrappers — one per public method on `slyng_client::Client`.
//!
//! Every command pattern is the same: build (or reuse) the lazy
//! `slyng_client::Client` for `api_host`, forward the call, return the
//! typed struct from `slyng-types`. Tauri's IPC layer serialises the
//! struct through serde so the JS side receives a plain object whose
//! shape matches `@slyng/client/wasm`'s `.d.ts`.
//!
//! No more `Value` round-trips — every endpoint is typed end-to-end.

use crate::auth::ClientHandle;
use serde_json::Value;
use slyng_client::Identity;
use slyng_types::{
...
async fn client<R: Runtime>(
	...
	api_host: &str,
) -> Result<slyng_client::Client, String> {
...
fn _ensure_inner_types_in_scope(
	...
	_e: slyng_types::MemberMessageEntry,
) {
}
