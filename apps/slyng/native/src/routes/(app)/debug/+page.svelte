<script lang="ts">
	...
	function refresh() {
		...
		injected = {
			top: cs.getPropertyValue('--slyng-sai-top').trim() || '(unset)',
			right: cs.getPropertyValue('--slyng-sai-right').trim() || '(unset)',
			bottom: cs.getPropertyValue('--slyng-sai-bottom').trim() || '(unset)',
			left: cs.getPropertyValue('--slyng-sai-left').trim() || '(unset)'
		};
		if (probe) {
...
matchMedia('(max-width: 767px)') = {mobile}</pre>
	...
	<section class="rounded-md border border-border p-3 text-xs">
		<h2 class="mb-1 font-semibold text-sm">Native-injected (--slyng-sai-*)</h2>
		<pre class="font-mono">--slyng-sai-top    = {injected.top}
--slyng-sai-right  = {injected.right}
--slyng-sai-bottom = {injected.bottom}
--slyng-sai-left   = {injected.left}</pre>
