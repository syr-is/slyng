/**
 * Native `SlyngClient` implementation. One `invoke('command_name', args)`
 * per method, no WASM in the WebView. Types come from `@slyng/client`,
 * which itself re-exports them from the wasm-pack `.d.ts` (auto-derived
 * from `slyng-types`'s Rust structs via tsify-next).
 *
 * The shape mirrors `@slyng/app-core/api`'s contract exactly: every
...
/** Build a `SlyngClient` whose calls go through Tauri IPC for the given host. */
export function createNativeApi(apiHost: string): SlyngClient {
