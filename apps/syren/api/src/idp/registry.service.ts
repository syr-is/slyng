...
import { canonicalize, encodeMultibase } from '@syren/idp-crypto';
import type { OwnedOutboxJob, OwnedRegistry, RegistrySyncResult } from '@syren/types';
 ...
 *
 * syr signs client-side (its server never holds the root key); syren stores the
...
export class RegistryService {
	...
	async sync(did: string, password: string): Promise<RegistrySyncResult> {
		...
		try {
			await rootSign('syren-registry-probe');
		} catch {
