...
/**
 * Platform delegation server — the IdP side of the flow syren itself
 * consumes as a client (auth.service.ts). External platforms (other
 * syrens, syr-family apps) authenticate local users through these
...
export class PlatformController {
	...
	// ── User-facing management (syren session auth) ───────────────────
	...
	// ── Consent (syren session auth; SPA-driven) ──────────────────────
