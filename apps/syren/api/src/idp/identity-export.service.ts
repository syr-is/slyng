...
import { canonicalize, encodeMultibase, sign } from '@syren/idp-crypto';
import { extractLocalId } from '@syren/types';
import type { IdentityExportManifest, IdentityExportCounts } from '@syren/types';
...
export class IdentityExportService {
	...
	async exportIdentity(did: string, password: string): Promise<{ zip: Uint8Array; filename: string }> {
		...
		return { zip, filename: `syren-identity-${account.username}-${stamp}.zip` };
	}
