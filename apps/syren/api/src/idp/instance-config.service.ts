...
import type { InstanceLimits } from '@syren/types';
 ...
 * the kv `instance_config` table (env `SYREN_MAX_FILE_SIZE_MB` /
 * `SYREN_STORAGE_LIMIT_GB` seed the defaults, so a fresh instance works with no
 * kv rows):
 *
	...
	private async read(): Promise<{ maxFileMb: number; storageGb: number }> {
		...
		const maxFileMb =
			maxFile?.value && maxFile.value > 0
				? maxFile.value
				: this.envNum('SYREN_MAX_FILE_SIZE_MB', DEFAULT_MAX_FILE_SIZE_MB);
		const storageGb =
			storage?.value && storage.value > 0
				? storage.value
				: this.envNum('SYREN_STORAGE_LIMIT_GB', DEFAULT_STORAGE_LIMIT_GB);
