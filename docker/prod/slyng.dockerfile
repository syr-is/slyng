...
ARG SLYNG_API_URL=https://slyng.gg

ENV PUBLIC_URL=${PUBLIC_URL}
ENV SLYNG_API_URL=${SLYNG_API_URL}

COPY apps/slyng ./apps/slyng
COPY packages ./packages

# Build workspace packages in dependency order. `pnpm --filter` runs
# each package's own `build` script, which for @slyng/client invokes
...
RUN pnpm --filter @slyng/client build
RUN pnpm --filter @slyng/app-core build
RUN pnpm --filter @slyng/ui build
...
COPY --from=builder --chown=sveltekit:nodejs /app/apps/slyng/web/build ./build
