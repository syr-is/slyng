...
# The root package.json declares @slyng/app-core, @slyng/ui, and
# @slyng/types as workspace dependencies, so pnpm install validates
# the whole workspace graph even with --filter. Every package.json
# referenced transitively must be staged — including @slyng/client
# (transitively required by @slyng/app-core) — or pnpm refuses to
...
# recent Rust toolchain transitively. Needed for @slyng/idp-crypto's
# WASM build (vendored syr crypto crates). Same pattern as
# docker/prod/slyng.dockerfile.
...
# slyng-idp-crypto without the sibling crates present.
...
RUN pnpm --filter @slyng/idp-crypto build
RUN pnpm --filter @slyng/api build
...
RUN pnpm --filter @slyng/api --prod deploy pruned
...
COPY --from=builder --chown=nestjs:nodejs /app/apps/slyng/api/dist ./dist
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=10s --retries=3 \
    CMD node -e "require('http').get('http://localhost:' + (process.env.SLYNG_API_PORT || 5175) + '/reference', (r) => {process.exit(r.statusCode === 200 ? 0 : 1)}).on('error', () => process.exit(1))" || exit 1
